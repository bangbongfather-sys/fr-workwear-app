...
async function toolGetSection(args, env) {
  ...
  if (section === "_version") {
    return { content: [{ type: "text", text: JSON.stringify({ build: "2026-07-13-hover-peek-fix", note: "호버 프리뷰 수정 — 좁은 창/터치 겸용 노트북에서도 표시(진짜 터치 전용 기기만 숨김), 금·토 일정 프리뷰 화면 밖 잘림 방지"}) }] };
  }
