...
async function toolGetSection(args, env) {
  ...
  if (section === "_version") {
    return { content: [{ type: "text", text: JSON.stringify({ build: "2026-07-23-po-share-pdf", note: "발주서 공유·다운로드를 PDF로 통일 — 공유 시 이미지(PNG) 대신 A4 PDF 파일을 카톡·메일·문자로 전송(미지원 환경은 PDF 저장 폴백)"}) }] };
  }
