...
- **계좌는 2개로 시작**: 나정엔터프라이즈 계좌 1개(`biz:"nj"`) + 엔제이세이프티(법인) 계좌 1개(`biz:"corp"`). 사업자번호가 다르므로 팝빌 연동회원도 2개가 된다. 나중에 계좌를 늘릴 때는 `POPBILL_ACCOUNTS` secret에 한 줄만 추가하면 된다.
...
Cloudflare Worker가 매일 정해진 시간에 팝빌 REST API(SDK 대신 `fetch` + WebCrypto로 직접 호출)로 KB 계좌 2개(나정엔터프라이즈·엔제이세이프티 법인)의 **입금**만 수집해서, 앱 데이터와 분리된 **수집함(`/frw_bank_inbox`)** 에 쌓는다. 앱이 열릴 때 수집함을 기존 `bankDeposits`에 **멱등(idempotent) 병합**하고, 기존 거래처 추천 로직을 강화해 자동 매칭한다. 엑셀 업로드는 그대로 두되 자동수집분과 겹치면 건너뛴다.
...
[{ "id":"kb-nj",   "corpNum":"나정엔터프라이즈 사업자번호",   "bankCode":"0004", "accountNumber":"…", "biz":"nj",   "alias":"나정 국민" },
 { "id":"kb-corp", "corpNum":"엔제이세이프티 법인사업자번호", "bankCode":"0004", "accountNumber":"…", "biz":"corp", "alias":"법인 국민" }]
```
...
1. KB 계좌 2개(나정·법인)의 입금이 **사람 손 없이 평일 매시간** `bankDeposits`에 들어오고, 계좌마다 사업자 탭(나정/법인)으로 자동 분류된다.
...
- 계좌 2개를 테스트 환경에 등록 → `wrangler dev` + `/__scheduled` 트리거로 수집.
- 확인할 것:
...
**답을 받은 것:** 계좌 2개(나정엔터프라이즈 1 + 엔제이세이프티 법인 1) · 입금 처리는 직접 확인 후 처리 · 실시간은 「평일 매시간 + 지금 수집 버튼」으로.

**남은 질문**

1. **팝빌 연동 신청(LinkID/SecretKey 발급)은 되어 있나요?** 정액제는 **계좌당 월 5,000원 × 2 = 월 10,000원** (VAT 별도 여부 미확인)입니다. 두 사업자 모두 팝빌 회원가입이 필요합니다.
